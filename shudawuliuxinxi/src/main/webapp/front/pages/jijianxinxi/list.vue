<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.jijianxinxiName" type="text" placeholder="收件人名称"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in jijianxinxiTypesList" :key="index"
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-title"
            ...
            {{product.jijianxinxiName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"80rpx","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.jijianxinxiPhoto?product.jijianxinxiPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.jijianxinxiNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#fff","textAlign":"center"}'>
            <text class="">￥{{product.jijianxinxiNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;"
		 v-if="product.yonghuId == user.id">
            <text v-if="isAuth('jijianxinxi','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('jijianxinxi','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('jijianxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	let jijianxinxiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'jijianxinxi_types'
		  	});
		  	this.jijianxinxiTypesList = jijianxinxiTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.jijianxinxiName = "";
			},
			//类别搜索
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.jijianxinxiTypes = this.categoryName
                            }

                let res = await this.$api.list(`jijianxinxi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jijianxinxi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.jijianxinxiUuidNumber) {
                    searchForm['jijianxinxiUuidNumber'] = this.searchForm.jijianxinxiUuidNumber
                }
				if (this.searchForm.jijianxinxiName) {
                    searchForm['jijianxinxiName'] = this.searchForm.jijianxinxiName
                }
				if (this.searchForm.jijianxinxiName) {
					searchForm['jijianxinxiName'] = this.searchForm.jijianxinxiName
				}

                let res = await this.$api.list(`jijianxinxi`, searchForm);
				// 如果是第一页数据置空
