		...
		<view class="container">
			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<!-- 名字-->
					<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.jijianxinxiName}}
					</view>
				</view>
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop" v-if="detail.yonghuId == user.id">
				...
				<view style="text-align: right;">
					...
					<button @tap="onSubmit" style="margin-right: 10upx;" class="cu-btn bg-red round shadow-blur"
                            :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[3],borderColor:btnColor[3]}">申请发票</button>
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			async onSubmit(){
				let data = {
					jijianxinxiId:this.detail.id
				}
				await this.$api.save(`fapiaoshenqing`, data);
				this.$utils.msg("申请成功")
			},
			//下载
			download(url) {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('jijianxinxi', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.jijianxinxiPhoto ? this.detail.jijianxinxiPhoto.split(",") : [];
			},
